...
Caches raw + parsed outputs to ~/.cache/find-papers/<canonical_id>/.
...
Optional env vars:
  ...
  NCBI_TOOL                   PubMed/PMC tool name (default: find-papers-skill)
  FIND_PAPERS_CACHE           Cache root (default: ~/.cache/find-papers)
  RESEARCH_PAPERS_CACHE       Legacy cache root override
...
CACHE_ROOT = Path(
    os.environ.get(
        ...
        os.environ.get(
            ...
            str(Path.home() / ".cache" / "find-papers"),
        ),
    )
)
USER_AGENT = "find-papers-skill/1.0"
DEFAULT_TIMEOUT = 30
...
def try_pmc(pid: PaperID, result: FetchResult) -> bool:
    ...
    params = {
        ...
        "rettype": "full",
        "retmode": "xml",
        "tool": os.environ.get("NCBI_TOOL", "find-papers-skill"),
    }
